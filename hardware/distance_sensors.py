...
class DistanceSensors(Component):
    ...
    This version has been modified for a single center sensor.
...
#          Orientation.STBD: self._stbd_sensor
    ...
    def __getattr__(self, name):
        ...
        Access individual sensors via attributes, e.g.,
            ...
            _cntr_sensor = _sensors.CNTR
        '''
        if name in Orientation.__members__: # check if the name is a valid orientation
            orientation = Orientation[name]
            if hasattr(self, '_{}_sensor'.format(orientation.name.lower())):
                return getattr(self, '_{}_sensor'.format(orientation.name.lower()))

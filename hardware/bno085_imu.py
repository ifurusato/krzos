...
class Bno085(BNO085, Component):
    ...
    def is_calibrated(self):
        '''
        Returns True if magnetometer accuracy ≥ threshold.
        '''
...
#           BNO085.enable(self)
    ...
    def bench_calibrate(self):
        ...
        while _count < _limit:
            ...
            self._log.info('🌰 mag accuracy: {}; stdev: {:.4f}; is calib: {}; len: {}, threshold: {}'.format(\
                    self._magnetometer_accuracy, self._stdev, self.is_calibrated, len(self._queue), self._stability_threshold))
            # check completion criteria
            if self.is_calibrated \
                        and len(self._queue) > 50 \
                        and self._stdev > 0.0 \
                        and self._stdev < self._stability_threshold:
